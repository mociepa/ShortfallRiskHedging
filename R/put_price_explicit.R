...
put_price_explicit <- function(asset, strike, rate, vol, drift, p, time, End_Time, L, L2 = NA){
  if (p == 1){
    ...
    result <- put_price_concave(asset, strike, rate, vol, drift, p, time, End_Time, L, L2 = NA)
  }
  
  else{
    result <- put_price_convex(asset, strike, rate, vol, drift, p, time, End_Time, L, L2 = NA)
  }
