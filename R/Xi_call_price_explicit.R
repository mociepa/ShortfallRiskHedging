...
Xi_call_price_explicit <- function(asset, strike, rate, vol, drift, p, time, End_Time, L, L2 = NA){
  if (p <= 0){
    stop("Wrong p argument. p > 0")
  }

  else if (p == 1){
    result <- Xi_call_price_linear(asset, strike, rate, vol, drift, time, End_Time, L)
  }

  else if (p < 1){
    result <- Xi_call_price_concave(asset, strike, rate, vol, drift, p, time, End_Time, L, L2)
  }

  else{
    result <- Xi_call_price_convex(asset, strike, rate, vol, drift, p, time, End_Time, L, L2)
  }
  return(result)
}
